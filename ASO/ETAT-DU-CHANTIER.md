> **24 sept. 2026 — 6.4 (build 29) soumise** : fiche ASO (« WiFi Heat Map & Analyzer ») en 50 langues, app traduite en 49 langues, frises iPhone + iPad paysage. Publication automatique à l'approbation. Script : `tools/publish-6.4.py`. 25 sept. : la 6.4 est en vente ; **6.4.1 (build 30) soumise** pour corriger la capture iPhone « 2_measure » (carte absente) en en-US/GB/AU/CA, nl-NL et sk — même code, notes « corrections et améliorations mineures » (`release_notes_6.4.1.txt`). Script : `ASC_VERSION=6.4.1 ASC_BUILD=30 ASC_WHATS_NEW_FILE=release_notes_6.4.1.txt tools/publish-6.4.py <étape>`. Une nouvelle version reprend automatiquement les captures de la précédente : seules les langues modifiées sont à retéléverser. 26 sept. : 6.4.1 en vente ; **6.4.2 (build 31) soumise** : sous-titres et mots-clés des 50 langues sans « signal / puissance / dBm » (l'app iOS mesure le débit et la latence, pas le RSSI). Traductions générées sans relecture native : à faire relire en priorité pour les langues indiennes, le thaï, l'ourdou et le malais.
...
- **Mode démo captures** : `DemoData.swift`, arguments `-UIScreenshotMode` + `-UIScreenshotScenario welcome|heatmap|measure|discover|details|3d|traces`, navigation auto, données fictives (sondes, courbe de latence, journal), aucune dépendance réseau.
...
- `scripts/screenshots.sh` : captures auto (simulateurs dédiés « ASC iPhone 6.9 » / « ASC iPad 13 » créés par le script, barre d'état 9:41, 6 écrans pour les 7 locales : accueil, mesure, découverte, détails, 3D, traces). Formats : iPhone 6,9" portrait, iPad 13" portrait **et paysage** (`ipad13-landscape`, rotation par `scripts/SimRotator`), Mac 16:10.
- **Décision (24 sept. 2026) : on publie les frises** (`<locale>/<appareil>-frise/`, produites par `scripts/frise-screenshots.swift`) et non les captures à bandeau : appareils dessinés en perspective sur un fond continu, comme la frise de la 6.1. 6 écrans × 7 locales, légendes dans `scripts/screenshot-captions.tsv` (nl-NL, it et en-GB réutilisent l'interface anglaise).
- **Décision (24 sept. 2026) : l'iPad est publié en paysage** (2752×2064, mise en page réelle en deux colonnes), l'iPhone en portrait ; une seule orientation par jeu, jamais de mélange. Les captures iPad portrait restent produites (passe préalable du script) mais ne sont pas à téléverser.
