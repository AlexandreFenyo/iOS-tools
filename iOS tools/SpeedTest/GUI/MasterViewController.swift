...
//        let (index_paths_removed, index_paths_inserted) = DBMaster.shared.addNode(node)
    ...
    func floodUDP(_ address: IPAddress) {
        ...
        Task.detached(priority: .userInitiated) {
            await self.popUp("UDP flood", "UDP flooding sends packets asynchronously to UDP port 8888 of the target at a maximum rate, but many packets can be lost at software, hardware or network stages. Note that the throughput that is displayed on this chart is the one achieved at the software layer of your device. Therefore, it certainly is above the one at which data is sent over the network: you must use a tool to estimate the reached bandwitdh. Either sniff the network or count packets on the target, for instance.", "I understand")
