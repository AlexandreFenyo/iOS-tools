...
struct XXXStepByStepSwiftUIView: View {
    init(_ step_by_step_view_controller: StepByStepViewController) {
        self.step_by_step_view_controller = step_by_step_view_controller
        self.stepByStepPhotoController = StepByStepPhotoController(
            step_by_step_view_controller: step_by_step_view_controller)
    }

    @Environment(\.horizontalSizeClass) private var horizontalSizeClass

    let stepByStepPhotoController: StepByStepPhotoController
    weak var step_by_step_view_controller: StepByStepViewController?
            ...
            VStack {
                HStack {
                    ...
                    Text("Heat Map Builder")
                        ...
                        .padding()
                        .sheet(
                            isPresented: $showing_map_picker,
                            onDismiss: { () -> Void in
                            ...
                            ImagePicker(
                                image: $model.input_map_image,
                                original_map_image: $model.original_map_image,
                                original_map_image_rotation: $model
                                    .original_map_image_rotation,
                                idw_values: $model.idw_values)
                        }
                    Spacer()
                }.background(Color(COLORS.toolbar_background))
                ...
                VStack {
                        ...
                        HStack(alignment: .top) {
                            ...
                            Button {
                                ...
                                Task {
                                    ...
                                    stepByStepPhotoController.saveImage(
                                        image: merged_image)
                                }
                            } label: {
                                VStack {
                    ...
                    if ENABLE_DEBUG_INTERFACE {
                        if toggle_help {
                            ...
                            .background(.gray)
                            .cornerRadius(15).padding(.bottom).padding(.leading)
                            .padding(.trailing)
                            .opacity(display_steps ? 1.0 : 0.8).animation(
                                .default, value: display_steps
                            )
                            .onChange(
                                of: cg_image_next,
                                perform: { _ in
                        ...
                        HStack {
                            VStack {
                                ...
                                Toggle(isOn: $toggle_preview) {
                                    Text("ignore white AP").font(.footnote)
                                        .foregroundColor(
                                            Color(COLORS.standard_background)
                                        ).frame(
                                            maxWidth: .infinity,
                            ...
                            VStack {
                                HStack {
                                    ...
                                    Slider(
                                        value: $power_scale,
                                        in: 0...POWER_SCALE_MAX)
                                }
                                HStack {
                                    Image(systemName: "slowmo").foregroundColor(
                                        Color(COLORS.standard_background))
                                    Slider(
                                        value: $power_scale_radius,
                                        in: 1...POWER_SCALE_RADIUS_MAX)
                                }
                            }
                        }.padding()

                        Slider(
                            value: $power_blur_radius,
                            in: 1...POWER_BLUR_RADIUS_MAX)
                    }
